...
def send_request(req):
    try:
        ...
        s.settimeout(5) # Thêm timeout để tránh treo luồng nếu server lag
        s.connect((HOST, PORT))
        msg = json.dumps(req) + "\r\n"
...
def simulate_user_workflow(user_idx):
    ...
    """
    username = f"user_test_{user_idx}"
    password = "123"
    
    start_time = time.time()
    
    # 1. Login
    resp = login("teonhe2", "123456") # Dùng chung account hoặc tạo mới tùy bạn
