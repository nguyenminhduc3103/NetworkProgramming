...
void handle_list_tasks(int client, cJSON *data, int user_id, MYSQL *conn) {
    ...
    if (!pid || !cJSON_IsNumber(pid)) {
        write_server_log("[task/list_tasks] Missing project_id user_id=%d", user_id);
    ...
    if (!stmt || mysql_stmt_prepare(stmt, sql, strlen(sql)) != 0) {
        write_server_log("[task/list_tasks] user_id=%d project_id=%d", ERR_LIST_TASK_SERVER, "Server error", user_id, project_id);
    ...
    if (mysql_stmt_execute(stmt) != 0) {
        write_server_log("[task/list_tasks] status=%s message=%s user_id=%d project_id=%d", ERR_LIST_TASK_SERVER, "Server error", user_id, project_id);
        send_json_response(client, ERR_LIST_TASK_SERVER, "Server error", NULL);
...
void handle_create_task(int client, cJSON *data, int user_id, MYSQL *conn) {
    ...
    if (!pid || !cJSON_IsNumber(pid) || !tname || !cJSON_IsString(tname)) {
        write_server_log("[task/create_task] status=%s message=%s user_id=%d", ERR_CREATE_TASK_VAL, "Missing fields", user_id);
    ...
    if (!stmt || mysql_stmt_prepare(stmt, sql, strlen(sql)) != 0) {
        write_server_log("[task/create_task] status=%s message=%s user_id=%d project_id=%d", ERR_CREATE_TASK_SERVER, "Server error", user_id, project_id);
    ...
    if (mysql_stmt_execute(stmt) != 0) {
        write_server_log("[task/create_task] status=%s message=%s user_id=%d project_id=%d task_name=%s", ERR_CREATE_TASK_SERVER, "Server error", user_id, project_id, tname->valuestring);
        send_json_response(client, ERR_CREATE_TASK_SERVER, "Server error", NULL);
        mysql_stmt_close(stmt);
        return;
    }
    ...
    write_server_log("[create_task] user_id=%d project_id=%d task_id=%d task_name=%s", RES_CREATE_TASK_OK, "Task created", user_id, project_id, task_id, tname->valuestring);
...
void handle_assign_task(int client, cJSON *data, int user_id, MYSQL *conn) {
    ...
    if (!tid || !cJSON_IsNumber(tid) ||
        ...
        write_server_log("[task/assign_task] status=%s message=%s user_id=%d", ERR_ASSIGN_TASK_VAL, "Missing fields", user_id);
    ...
    if (mysql_stmt_num_rows(q) == 0) {
        mysql_stmt_close(q);
        write_server_log("[task/assign_task] status=%s message=%s user_id=%d task_id=%d", ERR_ASSIGN_TASK_VAL, "Task not found", user_id, task_id);
    ...
    if (my_role != ROLE_PM) {
        write_server_log("[task/assign_task] status=%s message=%s user_id=%d task_id=%d", ERR_ASSIGN_TASK_PERM, "Permission denied", user_id, task_id);
    ...
    if (mysql_stmt_num_rows(su) == 0) {
        mysql_stmt_close(su);
        write_server_log("[task/assign_task] status=%s message=%s user=%s task_id=%d", ERR_ASSIGN_TASK_VAL, "Assigned user not found", assigned_username, task_id);
    ...
    if (mysql_stmt_execute(stmt) != 0 ||
        ...
        mysql_stmt_close(stmt);
        write_server_log("[task/assign_task] status=%s message=%s task_id=%d by user_id=%d", ERR_ASSIGN_TASK_SERVER, "Assign failed", task_id, user_id);
...
void handle_update_task(int client, cJSON *data, int user_id, MYSQL *conn) {
    ...
    if (!tid || !cJSON_IsNumber(tid) ||
        ...
        write_server_log("[task/update_task] status=%s message=%s user_id=%d", ERR_UPDATE_TASK_VAL, "Missing fields", user_id);
    ...
    if (mysql_stmt_num_rows(q) == 0) {
        mysql_stmt_close(q);
        write_server_log("[task/update_task] status=%s message=%s user_id=%d task_id=%d", ERR_UPDATE_TASK_VAL, "Task not found", user_id, task_id);
    ...
    if (role == ROLE_NONE) {
        write_server_log("[task/update_task] status=%s message=%s user_id=%d task_id=%d", ERR_UPDATE_TASK_PERM, "Permission denied", user_id, task_id);
    ...
    if (mysql_stmt_execute(stmt) != 0) {
        write_server_log("[task/update_task] status=%s message=%s task_id=%d by user_id=%d", ERR_UPDATE_TASK_SERVER, "Update failed", task_id, user_id);
...
void handle_comment_task(int client, cJSON *data, int user_id, MYSQL *conn) {
    ...
    if (!tid || !cJSON_IsNumber(tid) ||
        ...
        write_server_log("[task/comment_task] status=%s message=%s user_id=%d", ERR_COMMENT_TASK_PERM, "Missing fields", user_id);
    ...
    if (mysql_stmt_num_rows(q) == 0) {
        mysql_stmt_close(q);
        write_server_log("[task/comment_task] status=%s message=%s user_id=%d task_id=%d", ERR_COMMENT_TASK_PERM, "Task not found", user_id, task_id);
    ...
    int role = db_get_user_role(conn, user_id, project_id);
    if (role <= ROLE_NONE) {
        write_server_log("[task/comment_task] status=%s message=%s user_id=%d task_id=%d", ERR_COMMENT_TASK_PERM, "Permission denied", user_id, task_id);
    ...
    if (mysql_stmt_execute(stmt) != 0) {
        mysql_stmt_close(stmt);
        write_server_log("[task/comment_task] status=%s message=%s task_id=%d by user_id=%d", ERR_COMMENT_TASK_SERVER, "Comment failed", task_id, user_id);
