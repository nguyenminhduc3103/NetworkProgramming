...
void *client_thread(void *arg) {
    ...
    while (1) {
        ...
        if (!root) {
            write_server_log("[PARSE] Invalid JSON from %s:%d", client_ip, client_port);
        ...
        if (!action || !cJSON_IsString(action) || !data || !cJSON_IsObject(data)) {
            write_server_log("[PARSE] Missing action or data from %s:%d", client_ip, client_port);
        ...
        if (strcmp(act, "login") != 0 && strcmp(act, "register") != 0) {
            ...
            if (!session || !cJSON_IsString(session)) {
                write_server_log("[AUTH] Missing session from %s:%d", client_ip, client_port);
            ...
            if (user_id < 0) {
                write_server_log("[AUTH] Invalid session from %s:%d", client_ip, client_port);
        ...
        if (strcmp(act, "register") == 0) {
            ...
            if (!data || !cJSON_IsObject(data)) {
                write_server_log("[DISPATCH] list_members missing data user_id=%d", user_id);
            ...
            if (!pid || !cJSON_IsNumber(pid)) {
                write_server_log("[DISPATCH] list_tasks missing project_id user_id=%d", user_id);
            ...
            int role = db_get_user_role(conn, user_id, pid->valueint);
            if (role == ROLE_NONE) {
                write_server_log("[DISPATCH] list_tasks DENIED - permission denied user_id=%d project_id=%d", user_id, proj_id);
